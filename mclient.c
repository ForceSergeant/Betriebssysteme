...
int main (int argc, char **argv) {
	...
	//Übersende Anzahl der Nachbarn
	strAnzNachbar = argc - 2;
	printf("argc: %d\n", strAnzNachbar);	
	retval = sendto (socketval, ("anzahl%d", &strAnzNachbar), sizeof(strAnzNachbar), 0, (struct sockaddr *) &serverinfo, sizeof (serverinfo));
	if(retval < 0) {
		perror("Fehler ist: ");
	}
